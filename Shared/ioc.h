...
typedef enum _REPORT_ID
{
	...
	REPORT_ID_SIGNATURE,
	REPORT_ID_HASH,
	REPORT_ID_BLOCKED_PROCESS,
	REPORT_ID_BLOCKED_THREAD
...
typedef struct _REPORT_SIGNATURE
{
	USHORT SigIndex;
	ULONG64 PageStart;
	BYTE PageHash[MD5_SIZE]; // MD5
	UCHAR PageData[PAGE_SIZE];
}REPORT_SIGNATURE, * PREPORT_SIGNATURE;

typedef struct _REPORT_BLOCKED_PROCESS
{
	HANDLE ProcessId;
	CHAR ImageName[15];
}REPORT_BLOCKED_PROCESS, * PREPORT_BLOCKED_PROCESS;

typedef struct _REPORT_BLOCKED_THREAD
{
	HANDLE ProcessId;
	HANDLE ThreadId;
	CHAR ImageName[15];
}REPORT_BLOCKED_THREAD, * PREPORT_BLOCKED_THREAD;
#pragma pack(pop)
